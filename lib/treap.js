...
;(function(module) {
  ...
  var protoTreap = {
    ...
    insert: function(key, data) {
      ...
      var node = {key: key, _priority: Math.random(), size: 1};
      ...
      for (var s = node._parent; s; s = s._parent) s.size++;
  ...
  , traverse: function(fn) {
      var trav = function(node) {
        ...
        return trav(node.left) || fn(node) || trav(node.right);
      };
  ...
  , _parSide: function(node) {  
       return (node._parent)
        ? (node._parent.left && node._parent.left.key === node.key) 
          ? 'left' 
          : 'right'
        : null;
    }
  ...
  , _rotate: function(pivot) {
      ...
      pivRoot.size = 1 + pivRoot.left.size + pivRoot.right.size;
